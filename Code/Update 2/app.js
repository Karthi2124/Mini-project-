document.addEventListener("DOMContentLoaded", () => {
    ...
    function processUserResponse(message) {
        if (currentSymptomBeingProcessed) {
            ...
            if (questionIndex < symptomData.questions[userLanguage].length) {
                ...
                let completedSymptom = currentSymptomBeingProcessed;
                ...
                if (currentSymptoms.length > 1) {
                    currentSymptoms.shift();
                    currentSymptomBeingProcessed = currentSymptoms[0];
                    speak(symptoms[currentSymptomBeingProcessed].questions[userLanguage][0]);
                }
            }
            return;
        }
    ...
    darkModeToggle.addEventListener("click", () => {
        ...
        let mode = document.body.classList.contains("dark-mode") ? "Dark Mode Enabled" : "Light Mode Enabled";
        speak(mode);
    });
});
